...
function createDonutChart(data, testName, query) {
 ...
 function getSetupFunction(data) {
    return function() {
        // Donut chart
        createDonutChart(data.cpu_util, "CPU", ".donut.overview#cpu-util")
        createDonutChart(data.cpu_iowait, "CPU", ".donut.overview#cpu-iowait")
        createDonutChart(data.mem_used, "CPU", ".donut.overview#mem-used")
        createDonutChart(data.disk_util, "CPU", ".donut.overview#disk-util")

        // Disk:
        //  xvda1
        Morris.Bar({
            element: 'disk-response-overview',
            data: [
        ...
        var overviewAggregateChart = c3.generate({
            bindto: '.chart.overview#aggregate',
            data: {
                columns: [
